...
<div class="max-w-3xl mx-auto my-12 px-4 sm:px-6 lg:px-8 article">
    <div class="text-center">
        <h2 class="text-4xl mb-4 font-black text-gray-900 ">{{$article->title}}</h2>
    ...
    <div class="mt-6 prose prose-indigo  min-w-full article">
        {!! Str::of($article->body)->markdown() !!}
    </div>
    <div class="mt-6 border  rounded-md p-4 flex items-center text-sm text-light">
        <img class="w-20 h-20 rounded-full object-cover" src="{{$article->user->profile_photo_url}}" alt="{{$article->user->name}}">
        <div class="ml-4">
            <div class="text-base text-gray-700 ">{{__("By:")}} <a class="font-bold" href="{{route('author.filter', ['user' => $article->user->id])}}">{{$article->user->name}}</a></div>
            @if(isset($article->user->bio))
                <div class="mt-1 text-sm text-gray-600 ">{{$article->user->bio}}</div>
            @endif
